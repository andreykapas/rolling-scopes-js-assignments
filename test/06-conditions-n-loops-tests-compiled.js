...
describe('06-conditions-n-loops-tasks', function () {
    ...
    it.optional('getFizzBuzz should return the output value according specification', () => {
        [1, 2, 4, 7, 8, 11, 13, 14, 16, 17, 19, 22, 23, 26, 28, 29, 31, 32, 34, 37, 38, 41, 43, 44, 47, 49, 52, 53, 56, 58, 59, 61, 62, 64, 67, 68, 71, 73, 74, 76, 77, 79, 82, 83, 86, 88, 89, 91, 92, 94, 97, 98].forEach(num => {
            var actual = tasks.getFizzBuzz(num);
            assert.equal(actual, num, `getFizzBuzz shoud return ${ num } for ${ num }, but actually ${ actual }`);
        });

        [3, 6, 9, 12, 18, 21, 24, 27, 33, 36, 39, 42, 48, 51, 54, 57, 63, 66, 69, 72, 78, 81, 84, 87, 93, 96, 99].forEach(num => {
            var actual = tasks.getFizzBuzz(num);
    ...
    it.optional('getFactorial should return the functorial of given number', () => {
        [{ n: 1, expected: 1 }, { n: 5, expected: 120 }, { n: 10, expected: 3628800 }].forEach(data => {
    ...
    it.optional('getSumBetweenNumbers should return the sum inside the specified interval', () => {
        [{ n1: 1, n2: 2, expected: 3 }, { n1: 5, n2: 10, expected: 45 }, { n1: -1, n2: 1, expected: 0 }].forEach(data => {
    ...
    it.optional('isTriangle should check if triangle can be built', () => {
        [{ sides: [1, 2, 3], expected: false }, { sides: [3, 4, 5], expected: true }, { sides: [10, 1, 1], expected: false }, { sides: [10, 10, 10], expected: true }].forEach(data => {
            [[0, 1, 2], [0, 2, 1], [1, 2, 0], [1, 0, 2], [2, 0, 1], [2, 1, 0]].forEach(idx => {
                var actual = tasks.isTriangle(data.sides[idx[0]], data.sides[idx[1]], data.sides[idx[2]]);
                assert.equal(actual, data.expected, `Triangle from [${ data.sides.toString() }]: expected ${ data.expected } but actual ${ actual }`);
            });
        });
    });
    ...
    it.optional('doRectanglesOverlap should return true if rectangles overlap', () => {
        [{
            ...
            assert.equal(tasks.doRectanglesOverlap(data.rect1, data.rect2), data.expected, `doRectanglesOverlap(\n   ${ JSON.stringify(data.rect1) },\n   ${ JSON.stringify(data.rect2) }\n): expected ${ data.expected }`);
        });
    });
    ...
    it.optional('isInsideCircle should return true if point lies inside of the specified circle', () => {
        [{
            ...
            assert.equal(tasks.isInsideCircle(data.circle, data.point), data.expected, `isInsideCircle(\n   ${ JSON.stringify(data.circle) },\n   ${ JSON.stringify(data.point) }\n): expected ${ data.expected }`);
        });
    });

    it.optional('findFirstSingleChar should return the first unrepeated char from string', () => {
        [{ str: 'The quick brown fox jumps over the lazy dog', expected: 'T' }, { str: 'abracadabra', expected: 'c' }, { str: 'entente', expected: null }].forEach(data => {
            var actual = tasks.findFirstSingleChar(data.str);
            assert.equal(actual, data.expected, `First single char of '${ data.str }' = '${ data.expected }', but actual '${ actual }'`);
        });
    });
    ...
    it.optional('getIntervalString should return the string representation of math interval', () => {
        [{
            ...
            assert.equal(actual, data.expected, `getIntervalString(${ data.a }, ${ data.b }, ${ data.isStartIncluded }, ${ data.isEndIncluded }) shoud return '${ data.expected }', but actually '${ actual }'`);
        });
    });

    it.optional('reverseString should return the specified string in reverse order', () => {
        [{ str: 'The quick brown fox jumps over the lazy dog', expected: 'god yzal eht revo spmuj xof nworb kciuq ehT' }, { str: 'abracadabra', expected: 'arbadacarba' }, { str: 'rotator', expected: 'rotator' }, { str: 'noon', expected: 'noon' }].forEach(data => {
    ...
    it.optional('reverseInteger should return the specified number in reverse order', () => {
        [{ num: 12345, expected: 54321 }, { num: 1111, expected: 1111 }, { num: 87354, expected: 45378 }, { num: 34143, expected: 34143 }].forEach(data => {
    ...
    it.optional('isCreditCardNumber should validate CCN', () => {
        [79927398713, 4012888888881881, 5123456789012346, 378282246310005, 371449635398431, 378734493671000, 5610591081018250, 30569309025904, 38520000023237, 6011111111111117, 6011000990139424, 3530111333300000, 3566002020360505, 5555555555554444, 5105105105105100, 4111111111111111, 4012888888881881, 4222222222222, 5019717010103742, 6331101999990016, 54891243456789010].forEach(ccn => {
    ...
    it.optional('getDigitalRoot should return the cyclic sum of all digits', () => {
        [{ num: 12345, expected: 6 }, { num: 23456, expected: 2 }, { num: 10000, expected: 1 }, { num: 165536, expected: 8 }].forEach(data => {
    ...
    it.optional('toNaryString should return the n-ary string representation of number', () => {
        [{ num: 1024, n: 2, expected: '10000000000' }, { num: 6561, n: 3, expected: '100000000' }, { num: 365, n: 2, expected: '101101101' }, { num: 365, n: 3, expected: '111112' }, { num: 365, n: 4, expected: '11231' }, { num: 365, n: 5, expected: '2430' }, { num: 365, n: 6, expected: '1405' }, { num: 365, n: 7, expected: '1031' }, { num: 365, n: 9, expected: '445' }, { num: 365, n: 10, expected: '365' }].forEach(data => {
    ...
    it.optional('getCommonDirectoryPath should return the n-ary string representation of number', () => {
        [{
            ...
            assert.equal(actual, data.expected, `Common directory path fo [${ data.pathes }] = ${ data.expected }, but actual ${ actual }`);
        });
    });
    ...
    it.optional('getMatrixProduct should return the product of two specified matrices', () => {
        [{
            ...
            assert.deepEqual(actual, data.expected, `Product of [${ data.m1 }] x [${ data.m2 }] = [${ data.expected }], but actual ${ actual }`);
        });
    });
    ...
    it.optional('timespanToHumanString should return the human string representation of datetime period', () => {
        [{
            ...
            assert.equal(actual, data.expected, `timespanToHumanString('${ data.startDate }', '${ data.endDate }') shoud return '${ data.expected }', but actually '${ actual }'`);
        });
    });
    ...
    it.optional('evaluateTicTacToePosition should return the winner if defined', () => {
        ...
        [[[X, X, X], [O, O], [O,,]], [[, O, O], [X, X, X], [O,, O]], [[,, O], [O,, O], [X, X, X]], [[X,, O], [X,, O], [X, O]], [[O, X, O], [X, X, O], [O, X]], [[O, O, X], [X, O, X], [O, X, X]], [[X, O, O], [X, X, O], [O, X, X]], [[O, O, X], [X, X, O], [X,, O]]].forEach(data => {
        ...
        [[[O, O, O], [, X, X], [X,,]], [[X, X], [O, O, O], [X,, X]], [[,,], [X,, X], [O, O, O]], [[O,, X], [O, X, X], [O, X]], [[X, O, X], [X, O, O], [O, O, X]], [[X, X, O], [X, O, O], [, X, O]], [[O, X, X], [X, O, X], [O, X, O]], [[X, X, O], [X, O, X], [O,, X]]].forEach(data => {
            var actual = tasks.evaluateTicTacToePosition(data);
            assert.equal(actual, O, `Position: \n${ positionToSting(data) }\n  The winner is O, but actually '${ actual }'`);
        });

        [[[,,], [,,], [,,]], [[X,,], [O, O], [,, X]], [[X, O, X], [X, O, X], [O, X, O]], [[X, O, X], [O, X, X], [O, X, O]], [[X, O, X], [O,, O], [X, O, X]]].forEach(data => {
            var actual = tasks.evaluateTicTacToePosition(data);
            assert.equal(actual, undefined, `Position: \n${ positionToSting(data) }\n  The winner is undefined, but actually '${ actual }'`);
        });
    });
});

//# sourceMappingURL=06-conditions-n-loops-tests-compiled.js.map