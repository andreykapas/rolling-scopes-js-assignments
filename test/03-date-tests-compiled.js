...
describe('03-date-tasks', function () {
    ...
    it.optional('parseDataFromRfc2822 should parse rfc2822 string into a date value', function () {
        assert.equal(new Date(1995, 11, 17, 3, 24, 0).valueOf(), tasks.parseDataFromRfc2822('December 17, 1995 03:24:00').valueOf());

        assert.equal(1453816082000, tasks.parseDataFromRfc2822('Tue, 26 Jan 2016 13:48:02 GMT').valueOf());

        assert.equal(895373940000, tasks.parseDataFromRfc2822('Sun, 17 May 1998 03:00:00 GMT+01').valueOf());
    });

    it.optional('parseDataFromIso8601 should parse ISO 8601 string into a date value', function () {
        assert.equal(1453219657000, tasks.parseDataFromIso8601('2016-01-19T16:07:37+00:00').valueOf());

        assert.equal(1453190857000, tasks.parseDataFromIso8601('2016-01-19T08:07:37Z').valueOf());
    });
    ...
    it.optional('timeSpanToString should return the string represation of time span between two dates', function () {
        [{
            ...
            assert.equal(data.expected, tasks.timeSpanToString(data.startDate, data.endDate));
        });
    });
    ...
    it.optional('angleBetweenClockHands should returns the angle bettween clock hands for specified Greenwich datetime', function () {
        [{
            ...
            assert.equal(tasks.angleBetweenClockHands(new Date(data.date)), data.expected.toFixed(12), `Incorrect result for angleBetweenClockHands(${ new Date(data.date).toUTCString() }):`);
        });
    });
});

//# sourceMappingURL=03-date-tests-compiled.js.map